from src.areas_of_work.domain.entities import AreasOfWork, AreasOfWorkUpdate, AreasListResponse, AreasOfWorkResponse
...
@areas_router.get("/{area_id}", response_model=AreasOfWorkResponse, status_code=status.HTTP_200_OK)
def get_one_area(
        area_id: str,
        use_cases: AreaUseCases = Depends(get_areas_of_work_use_cases)
):
...
@areas_router.post("/", response_model=AreasOfWorkResponse, status_code=status.HTTP_201_CREATED)
def create_area(
        content: AreasOfWork,
...
@areas_router.patch("/{area_id}", status_code=status.HTTP_201_CREATED, response_model=AreasOfWorkResponse)
def update_area(
        area_id: str,
