...
def get_all(
        use_cases: EventList = Depends(get_event_list_cases)
):
    """ 
...
@event_list_router.get("/{id}", response_model=EventList, status_code=status.HTTP_200_OK)
def get_one(
        id: str,
...
@event_list_router.post("/", response_model=EventList, status_code=status.HTTP_201_CREATED)
def create(
        content: EventList,
...
@event_list_router.patch("/{id}", status_code=status.HTTP_201_CREATED, response_model=EventList)
def update(
        id: str,
