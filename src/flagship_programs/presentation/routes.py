from src.flagship_programs.domain.entities import FlagshipProgram, FlagshipProgramUpdate, FlagshipProgramResponse, FlagshipListResponse
...
@flagship_programs_router.get("/", response_model=FlagshipListResponse, status_code=status.HTTP_200_OK)
def get_all(
...
@flagship_programs_router.get("/{id}", response_model=FlagshipProgramResponse, status_code=status.HTTP_200_OK)
def get_one(
        id: str,
        use_cases: FlagshipUseCases = Depends(get_flagship_programse_cases)
):
...
@flagship_programs_router.post("/", response_model=FlagshipProgramResponse, status_code=status.HTTP_201_CREATED)
def create(
...
@flagship_programs_router.patch("/{id}", status_code=status.HTTP_201_CREATED, response_model=FlagshipProgramResponse)
def update(
        id: str,
