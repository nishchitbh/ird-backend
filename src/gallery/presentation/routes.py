from src.gallery.domain.entities import GalleryOut, GalleryUpload, GalleryList, GalleryUpdate
...
@gallery_router.post("/upload", status_code=status.HTTP_201_CREATED, response_model=GalleryOut)
...
@gallery_router.get("/gallery", status_code=status.HTTP_200_OK, response_model=GalleryList)
...
@gallery_router.get("/gallery/{gallery_id}", status_code=status.HTTP_200_OK, response_model=GalleryOut)
...
@gallery_router.patch("/gallery/{gallery_id}", status_code=status.HTTP_200_OK, response_model=GalleryOut)
async def update_gallery_by_id(
    ...
    try:
        return gallery_use_cases.update_gallery(gallery_id=gallery_id, gallery=gallery_data, current_user=current_user)
    except AppException as e:
        raise HTTPException(status_code=e.status_code, detail=e.detail)
...
async def delete_gallery_by_id(
    ...
    try:
        return gallery_use_cases.delete_gallery(current_user=current_user, gallery_id=gallery_id)
    except AppException as e:
        raise HTTPException(status_code=e.status_code, detail=e.detail)
